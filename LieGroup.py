...
def Phi(_vector):
	output_matrix = np.matrix([[0,0,0],[0,0,0],[0,0,0]])
...
def exp_SE2(_vector):  # 3*1 vector
	...
	_temp = V(_theta)*np.matrix([[_x], [_y]])

	return LieGroup(_theta, _temp[0,0], _temp[1,0])


def V(_theta):
	return (1.0/_theta)*np.matrix([[math.sin(_theta), -1+math.cos(_theta)],
